  ...
  if (confirmed) {
    return (
      <div className="min-h-screen bg-white text-black flex flex-col items-center justify-center px-6">
        ...
        >
          <div className="w-16 h-16 rounded-full bg-black/5 border border-black/15 flex items-center justify-center mx-auto mb-8">
            <CheckCircle2 className="w-8 h-8 text-black" />
          </div>
          <h1 className="font-serif text-3xl md:text-4xl text-black mb-4">Rendez-vous confirmé</h1>
          <p className="text-black/60 font-sans text-sm leading-relaxed mb-2">
            {format(selectedDate!, 'EEEE d MMMM yyyy', { locale: fr })} à {selectedTime}
          </p>
          <p className="text-black/50 font-sans text-xs leading-relaxed mb-2">
            {format_ === 'physique' ? 'En physique dans les locaux du cabinet' : 'En visioconférence'}
          </p>
          <p className="text-black/50 font-sans text-xs leading-relaxed mb-10">
            Un membre de l'équipe Logan vous contactera par email pour confirmer les détails.
          </p>
          <Link to="/">
            <Button variant="outline" className="border-black/20 text-black hover:bg-black/5 rounded-sm px-6">
    ...
    <div className="min-h-screen bg-white text-black flex flex-col">
      ...
      <header className="px-6 sm:px-8 lg:px-10 flex items-center justify-between h-20 relative z-10">
        ...
        {step === 'calendar' ? (
          <Button
            variant="ghost"
            className="text-black/60 hover:text-black hover:bg-black/5 text-sm rounded-sm"
          ...
          <Link to="/">
            <Button variant="ghost" className="text-black/60 hover:text-black hover:bg-black/5 text-sm rounded-sm">
              <ArrowLeft className="w-4 h-4 mr-2" /> Retour
            </Button>
          </Link>
        )}
        ...
        <div className="w-full max-w-4xl">
          <motion.div variants={stagger} initial="hidden" animate="visible">
            ...
            <motion.div variants={fadeUp} className="text-center mb-12">
              <div className="inline-flex items-center gap-2 border border-black/15 rounded-sm px-3 py-1.5 mb-8">
                <span className="font-serif text-xs font-bold text-black/80 tracking-[0.1em]">LOGAN</span>
                <span className="w-px h-3 bg-black/20" />
                <span className="text-[9px] text-black/50 tracking-[0.12em] uppercase">Rendez-vous</span>
              </div>
              <h1 className="font-serif text-3xl md:text-5xl font-normal text-black mb-5 tracking-[-0.02em]">
                Échangeons ensemble
              </h1>
              <p className="text-base text-black/70 font-sans font-light max-w-xl mx-auto leading-relaxed mb-3">
              ...
              {step === 'form' && (
                <p className="text-sm text-black/50 font-sans font-light max-w-md mx-auto leading-relaxed">
            ...
            {step === 'calendar' && (
              <motion.div variants={fadeUp} className="grid md:grid-cols-2 gap-8 items-start">
                {/* Left: Calendar */}
                <div className="bg-black/[0.02] border border-black/[0.1] rounded-lg p-6">
                  <div className="flex items-center gap-2 mb-5">
                    <CalendarDays className="w-4 h-4 text-black/60" />
                    <span className="text-[10px] font-bold tracking-[0.12em] uppercase text-black/60">Choisir une date</span>
                  </div>
                  <Calendar
                    mode="single"
                ...
                <div className="space-y-6">
                  {selectedDate ? (
                    <>
                      <div className="bg-black/[0.02] border border-black/[0.1] rounded-lg p-6">
                        <div className="flex items-center gap-2 mb-5">
                          ...
                          <span className="text-[10px] font-bold tracking-[0.12em] uppercase text-black/60">
                      ...
                      {selectedTime && (
                        <motion.div
                          ...
                          className="bg-black/[0.02] border border-black/[0.1] rounded-lg p-6"
                        >
                          <div className="flex items-center gap-2 mb-5">
                            <Video className="w-4 h-4 text-black/60" />
                            <span className="text-[10px] font-bold tracking-[0.12em] uppercase text-black/60">Format du rendez-vous</span>
                          </div>
                          ...
                          <Button
                            ...
                            disabled={submitting}
                            className="w-full bg-black text-white hover:bg-black/90 font-sans text-sm font-bold rounded-sm py-5 group"
                          >
                          ...
                          <p className="text-[10px] text-black/40 mt-4 text-center leading-relaxed">
                    ...
                    <div className="bg-black/[0.02] border border-black/[0.1] rounded-lg p-10 text-center">
                      <CalendarDays className="w-8 h-8 text-black/30 mx-auto mb-4" />
                      <p className="text-sm text-black/50 font-sans font-light">
