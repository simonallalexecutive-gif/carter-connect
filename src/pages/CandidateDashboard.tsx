...
const CandidateHeader = () => {
  ...
  return (
    <header
      className="flex items-center justify-between border-b border-white/10 px-10 gap-6 flex-shrink-0 py-[34px]"
      style={{ background: '#0a0a0a' }}
    >
      <div className="flex items-center gap-5">
        ...
        {(store.prenom || store.nom) && (
          <div>
            ...
            <p className="text-[10px] font-sans font-medium text-white leading-none">
              {store.prenom} {store.nom}
            </p>
          </div>
        )}
      ...
      <div className="flex items-center gap-2 flex-wrap justify-end">
        {pills.map(p => (
          <span key={p} className="text-[10px] font-sans font-normal tracking-wide px-5 py-2.5 rounded-sm bg-white/10 text-white/80 border border-white/10 whitespace-nowrap">
            {p}
          </span>
        ))}
      </div>
