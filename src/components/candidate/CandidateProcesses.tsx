                  ...
                  <div className="mb-4 pb-4 border-b border-black/10">
                    ...
                    {offer.ranking && (
                      <div className="flex items-center gap-0 mb-3 flex-wrap">
                        <span className="text-[11px] font-sans text-black/45 leading-none">Cabinet {offer.nat || ''}</span>
                        <span className="mx-2 text-[11px] text-black/25">·</span>
                        <span className="text-[11px] font-sans text-black/45 leading-none">Chambers : Oui</span>
                      </div>
                    )}
                    {!offer.ranking && (
                      <div className="flex items-center gap-0 mb-3 flex-wrap">
                        ...
                        <span className="text-[11px] font-sans text-black/45 leading-none">Chambers : Non</span>
                      </div>
                    )}
                ...
                <div className="flex items-center gap-1 mb-4">
                  {STAGES.map((stage, i) => (
                    <div key={stage} className="flex items-center gap-1 flex-1">
                      <div className={`h-1.5 flex-1 rounded-full transition-colors ${i <= proc.stageIndex ? 'bg-black/70' : 'bg-black/10'}`} />
                      {i < STAGES.length - 1 && <ArrowRight className="w-3 h-3 text-black/20 shrink-0" />}
                    </div>
                  ))}
                </div>
                <div className="flex justify-between text-[9px] text-black/45 font-sans mb-4">
                  {STAGES.map((stage, i) => (
                    <span key={stage} className={`${i <= proc.stageIndex ? 'text-black/80 font-medium' : ''}`}>{stage}</span>
                  ))}
                </div>

                <div className="flex items-center justify-between">
                  <p className="text-[13px] font-sans text-black/60 leading-relaxed">{proc.note}</p>
                  <span className="text-[10px] text-black/35 font-sans shrink-0 ml-4">{new Date(proc.lastUpdate).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}</span>
                </div>
              </div>
            </div>
          );
