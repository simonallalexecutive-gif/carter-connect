    ...
    {MOCK_PROCESSES.length === 0 ? (
      ...
      <div className="space-y-4">
        {MOCK_PROCESSES.map((proc) => {
          ...
          return (
            <div key={proc.id} className="rounded-lg p-6 hover:shadow-[var(--shadow-elevated)] transition-shadow duration-300" style={{ background: 'hsl(220 40% 13%)' }}>
              ...
              <div className="flex items-center gap-1 mb-4">
                {STAGES.map((stage, i) => (
                  <div key={stage} className="flex items-center gap-1 flex-1">
                    <div className={`h-1.5 flex-1 rounded-full transition-colors ${i <= proc.stageIndex ? 'bg-white' : 'bg-white/20'}`} />
                    {i < STAGES.length - 1 && <ArrowRight className="w-3 h-3 text-white/20 shrink-0" />}
                  </div>
                ))}
              </div>
              <div className="flex justify-between text-[9px] text-white/50 font-serif mb-4">
                {STAGES.map((stage, i) => (
              ...
              <div className="flex items-center justify-between">
                <p className="text-[13px] font-serif text-white/60 leading-relaxed">{proc.note}</p>
                <span className="text-[10px] text-white/40 font-serif shrink-0 ml-4">{new Date(proc.lastUpdate).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}</span>
              </div>
            </div>
          );
        })}
