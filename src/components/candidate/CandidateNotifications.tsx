                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {notif.type === 'interest' && offer && (
                          <button
                            ...
                            className="inline-flex items-center gap-1 text-[10px] font-medium text-foreground hover:text-foreground/70 transition-colors underline underline-offset-2"
                          >
                ...
                {isExpanded && offer && (
                  <div className="border-t border-border bg-secondary/30 p-5">
                    <div className="mb-4">
                      ...
                      <div className="text-[10px] text-muted-foreground tracking-widest uppercase mb-2">{offer.reference}</div>
                      <p className="text-xs text-muted-foreground leading-relaxed">{offer.description}</p>
                      {offer.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-3">
                          {offer.tags.map((tag) => (
                            <span key={tag} className="text-[10px] px-2.5 py-1 rounded-full border border-border text-foreground/70">{tag}</span>
                          ))}
                        </div>
                      )}
                    </div>
                    ...
                    {isConfirmed ? (
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 bg-foreground text-background py-3 rounded-lg text-sm font-bold hover:bg-foreground/90 transition-colors"
                      >
