...
const MissionSection = () => (
  <section id="notre-approche" className="relative overflow-hidden bg-white">
    <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-8 lg:px-10 py-24 md:py-40">
      ...
      >
        {/* Cabinets */}
        <motion.div variants={fadeScale} className="group relative bg-black rounded-sm p-8 md:p-10">
          <div className="w-10 h-10 rounded-full border border-white/15 flex items-center justify-center mb-6">
            <Users className="w-4.5 h-4.5 text-white/50" strokeWidth={1.5} />
          </div>
          <h3 className="font-serif text-xl text-white mb-4 tracking-[-0.01em]">Pour les cabinets</h3>
          <p className="font-sans text-sm leading-[1.85] text-white/50 font-light">
        ...
        <motion.div variants={fadeScale} className="group relative border border-black/[0.08] rounded-sm p-8 md:p-10 hover:border-black/20 transition-colors duration-500">
          ...
          <h3 className="font-serif text-xl text-black mb-4 tracking-[-0.01em]">Pour les candidats</h3>
      ...
      >
        <motion.div variants={fadeUp} className="relative border-l-2 border-black/10 pl-8 py-2">
          <Handshake className="w-5 h-5 text-black/30 mb-4" strokeWidth={1.5} />
          <p className="font-sans text-[1.02rem] leading-[1.9] text-black/60 font-light">
            Dès lors qu'un rapprochement apparaît pertinent, nous intervenons comme intermédiaire afin de nous assurer de la justesse d'une rencontre et de la cohérence de vos projets respectifs. Nous vous accompagnons ensuite tout au long du processus de recrutement.
          </p>
        </motion.div>
      </motion.div>
