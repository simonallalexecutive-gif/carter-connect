...
const BenefitsSection = () => {
  return (
    <section className="py-16 md:py-20" style={{ background: 'hsl(40 30% 96%)' }}>
      <div className="carter-container">
        ...
        >
          <p className="text-xs font-sans font-medium tracking-[0.2em] uppercase text-black/40 mb-4 text-center">
            Nos engagements
          </p>
          <p className="text-sm text-black/50 font-light max-w-md leading-relaxed mx-auto text-center">
            Ce qui nous distingue
          </p>
        </motion.div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-black/[0.08] rounded-sm overflow-hidden max-w-5xl mx-auto">
          {engagements.map((item, i) => (
            <motion.div
              ...
              className="p-8 md:p-10 group hover:bg-black/[0.03] transition-colors duration-500"
              style={{ background: 'hsl(40 30% 96%)' }}
            >
              <div className="flex flex-col items-center gap-2 mb-5">
                <div className="w-10 h-10 rounded-lg bg-black/[0.06] flex items-center justify-center mx-auto group-hover:bg-black/[0.1] transition-colors duration-500">
                  <item.icon className="w-4.5 h-4.5 text-black/40 group-hover:text-black/70 transition-colors duration-500" />
                </div>
                <span className="text-[10px] font-mono text-black/20">
                  {String(i + 1).padStart(2, '0')}
                </span>
              </div>
              <h3 className="font-serif text-lg text-black mb-3 tracking-[-0.01em] leading-snug text-center">
                {item.label}
              </h3>
              <p className="text-[13px] font-sans text-black/55 font-light leading-relaxed text-center">
                {item.detail}
              </p>
            </motion.div>
          ))}
